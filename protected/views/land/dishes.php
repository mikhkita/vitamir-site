<? $inc = 0; $page = 0; foreach($dishes as $key => $item): ?>
    <?if($inc == 0): $page++; ?>
        <div class="b-page" data-page="<?=$page?>">
            <ul class="clearfix">
    <? endif; ?>
                <li class="dish-item clearfix" data-dish-id="<?=$item['id']?>" data-img="<?=$item['image']?>" data-name="<?=$item['name']?>" data-m-1="<?=$item['m_1']?>" data-m-2="<?=$item['m_2']?>" data-m-3="<?=$item['m_3']?>" data-w-1="<?=$item['w_1']?>" data-w-2="<?=$item['w_2']?>" data-w-3="<?=$item['w_3']?>" data-fat="<?=$item['fat']?>" data-pro="<?=$item['protein']?>" data-car="<?=$item['carbohydrate']?>" data-cal="<?=$item['calories']?>" data-pri="<?=$item['price']?>">
                    <h3><?=$item['name']?></h3>
                    <div class="b-image-cont">
                        <div class="dish-image b-image" style="background-image: url('<?=Yii::app()->baseUrl.'/'.$item['image']?>');" ></div>
                        <?if($item['action']): ?>
                    ...
                    <div class="clearfix b-price-cont">
                        ...
                        <a href='#' class="right fancy" data-block="b-popup-more">Подробнее</a>
